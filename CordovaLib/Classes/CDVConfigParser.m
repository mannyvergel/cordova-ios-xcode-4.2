...
- (void)parser:(NSXMLParser*)parser didStartElement:(NSString*)elementName namespaceURI:(NSString*)namespaceURI qualifiedName:(NSString*)qualifiedName attributes:(NSDictionary*)attributeDict
{
    if ([elementName isEqualToString:@"preference"]) {
        [settings setValue:[attributeDict objectForKey:@"value"] forKey:[[attributeDict objectForKey:@"name"] lowercaseString]];
    } else if ([elementName isEqualToString:@"feature"]) { // store feature name to use with correct parameter set
        featureName = [[attributeDict objectForKey:@"name"] lowercaseString];
    } else if ((featureName != nil) && [elementName isEqualToString:@"param"]) {
        NSString* paramName = [[attributeDict objectForKey:@"name"] lowercaseString];
        id value = [attributeDict objectForKey:@"value"];
        if ([paramName isEqualToString:@"ios-package"]) {
            [pluginsDict setValue:value forKey:featureName];
        }
